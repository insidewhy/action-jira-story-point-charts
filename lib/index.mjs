...
async function makeVelocityChart(pointBuckets, options) {
  const velocities = {
...
function buildMetrics(totalStoryPoints, metrics) {
  ...
  const diffPad = Math.max(...metrics.map(({ start, end }) => Math.floor(Math.log10(end - start)))) + 2;
  const diffPercentagePad = Math.max(
    ...metrics.map(
      ({ start, end }) => Math.round(Math.log10(end - start) / totalStoryPoints * 100)
    )
  ) + 3;
  return metrics.map(({ label, start, end }) => {
    const startRemaining = totalStoryPoints - start;
    const endRemaining = totalStoryPoints - end;
    return `> \`${(label + ":").padEnd(longestHeadingLength, " ")} ${startRemaining.toString().padStart(maxMetricLength, " ")} [${percentage(totalStoryPoints, startRemaining, percentagePad)}] -> ${endRemaining.toString().padStart(maxMetricLength, " ")} [${percentage(totalStoryPoints, endRemaining, percentagePad)}] (${(start - end).toString().padStart(
      diffPad,
      " "
    )} [${percentage(totalStoryPoints, start - end, diffPercentagePad)}])\``;
  }).join("\n");
}
function describeChanges(header, issues, timePeriod) {
...
async function runChartBot(options) {
  ...
  const byWeekChart = weeklyPointBuckets ? await makeRemainingStoryPointsLineChart(weeklyPointBuckets, options, "week") : void 0;
  const dailyPointBuckets = makePointBuckets(issues, DAY_IN_MSECS, 7);
  const byDayChart = dailyPointBuckets ? await makeRemainingStoryPointsLineChart(dailyPointBuckets, options, "day", 7) : void 0;
  const openIssuesChart = await makeOpenIssuesChart(issues, options);
  const weeklyVelocityChart = weeklyPointBuckets ? await makeVelocityChart(weeklyPointBuckets, options) : void 0;
  ...
  const initialCommentSections = [
    ...
    options.withWeeklyDescription && describeChanges(options.withWeeklyDescription, issues, WEEK_IN_MSECS)
  ].filter((v) => Boolean(v));
  if (channel) {
