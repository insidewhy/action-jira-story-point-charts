...
async function makeOpenIssuesChart(issues, options) {
  ...
  const sorted = [...openIssues.entries()].map(([status, stat2]) => ({ status, ...stat2 })).sort((a, b) => b.days - a.days);
  const theme = {
    xyChart: { plotColorPalette: `${statuses.inReview.color}, ${statuses.readyForQA.color}` }
  };
  const inReviewBar = sorted.map((stat2) => stat2.inReview ? stat2.days : 0);
  const readyForQABar = sorted.map((stat2) => stat2.inReview ? 0 : stat2.days);
  const mmd = `%%{init: {'theme': 'base', 'themeVariables': ${JSON.stringify(theme)}}}%%
xychart-beta
