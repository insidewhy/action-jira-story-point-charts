...
function buildMetrics(totalStoryPoints, metrics) {
  ...
  return metrics.map(({ label, start, end, velocities }) => {
    ...
    lineContent += `[${percentage(totalStoryPoints, endRemaining, percentagePad)}] (${(start - end).toString().padStart(diffPad, " ")} [${percentage(totalStoryPoints, start - end, diffPercentagePad)}])`;
...
async function runChartBot(options) {
  ...
  const weeklyVelocities = weeklyPointBuckets ? makePointBucketVelocities(weeklyPointBuckets) : void 0;
  const weeklyVelocityChart = weeklyVelocities ? await makeVelocityChart(weeklyVelocities, options) : void 0;
  ...
  const initialCommentSections = [
    ...
    options.withWeeklyDescription && describeChanges(options.withWeeklyDescription, issues, WEEK_IN_MSECS, weeklyVelocities)
  ].filter((v) => Boolean(v));
  if (channel) {
