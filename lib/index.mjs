...
function buildMetrics(startTotal, endTotal, metrics) {
  ...
  return enhancedMetrics.map(
    ({
      ...
      let lineContent = (label + ":").padEnd(longestHeadingLength, " ") + " " + startDisplay.toString().padStart(maxMetricLength, " ") + " [" + startPercentage.padStart(startPercentagePad) + "] -> " + endDisplay.toString().padStart(maxMetricLength, " ") + " [" + endPercentage.padStart(endPercentagePad) + "] (" + diffDisplay.toString().padStart(diffPad, " ") + " [" + diffPercentage.padStart(diffPercentagePad) + "])";
