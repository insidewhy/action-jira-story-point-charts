...
There are additional `sprint-burn-up`, `daily-work-item-changes`, `weekly-work-item-changes`, `velocity-by-developer`, `velocity-by-developer-this-week` and `velocity-by-developer-last-week` charts available which are not produced by default.
The `velocity-by-developer`, `velocity-by-developer-this-week` and `velocity-by-developer-last-week` charts need `developer` and `dev-complete-time` field configurations.
The `sprint-burn-up`, `daily-work-item-changes`, `weekly-work-item-changes` need the `store-work-item-history` option to be set.

The `sprint-burn-up` chart has a parameter and should be configured like this:
...
By default Monday through Friday are considered work days but this can be reconfigured with the `work-days` option.
This currently only affects the `sprint-burn-up` report.

```yaml
work-days: 123456
```

This defaults to `12345`, each digit represents a day from Sunday (`0`) to Saturday (`6`).
...
#### Reading historical ticket change data
...
An option can be used which will cause the action to store data in a directory structure so historical data can be read for each day.

```yaml
store-work-item-history: /tmp/action-jira-story-points-chart-data
```

This option is needed to support the `sprint-burn-up`, `daily-work-item-changes`, `weekly-work-item-changes` charts and will also improve the metrics generated by the `with-daily-description` and `with-weekly-description` textual reports.
...
Given that charts are being posted on a recurring basis into one or more slack channels, information about story points being added to a release can still be determined by comparing charts or by using the story point movement charts and/or the with-weekly-description`/`with-daily-description`options together with`store-work-item-history`.
